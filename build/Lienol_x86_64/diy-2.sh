...
sed -i "/uci commit network/i\uci set network.lan.gateway='192.168.8.1'" $ZZZ             # IPv4 网关
sed -i "/uci commit network/i\uci set network.lan.broadcast='192.168.8.255'" $ZZZ         # IPv4 广播
sed -i "/uci commit network/i\uci set network.lan.dns='61.177.7.1 114.114.114.114'" $ZZZ   # DNS(多个DNS要用空格分开)
...
sed -i "/uci commit system/i\uci set system.@system[0].hostname='OpenWrt Wen'" $ZZZ       # 修改主机名称为OpenWrt-123

sed -i "s/OpenWrt /WenJing build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" $ZZZ        # 增加自己个性名称281677160
